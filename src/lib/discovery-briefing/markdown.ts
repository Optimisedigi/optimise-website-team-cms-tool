 ...
 *
 * Section ordering matches the CMS-bound `DiscoveryBriefingForm`. Hidden
 * sections (via `state.hiddenSections`) are skipped along with their
 * preceding horizontal rule so the output stays clean.
 *
 * The standalone `public/client-discovery-briefing.html` form predates
 * this v3 restructure — it has its own legacy markdown emitter and is no
 * longer kept byte-for-byte in sync.
 */

import type {
  DiscoveryBriefingAudienceSegment,
  DiscoveryBriefingState,
} from "./types";
...
/** Check whether a stable section id is hidden by the user. */
...
/**
 * Render a single audience segment's four bold key/value lines beneath
 * its `### <name>` subheading.
 */
function pushAudienceSegment(
  lines: string[],
  segment: DiscoveryBriefingAudienceSegment,
): void {
  lines.push(`### ${segment.name || "Audience"}`, "");
  lines.push(
    `**Average order value:** ${segment.averageOrderValue || "Not provided"}`,
  );
  lines.push(
    `**Purchase frequency (per client per year):** ${segment.purchaseFrequency || "Not provided"}`,
  );
  lines.push(
    `**Current new leads / month:** ${segment.newLeadsPerMonth || "Not provided"}`,
  );
  lines.push(
    `**Ideal lead volume / month:** ${segment.idealLeadVolume || "Not provided"}`,
  );
}
...
export function buildDiscoveryBriefingMarkdown(
  ...
  /**
   * Push the section's body, prefixed by a `---` rule unless this is the
   * very first emitted section. Skips entirely when the section is
   * hidden, and rolls back the rule+blank when the body chose to emit
   * nothing (used for the conditional Lead Magnets / RACI / Additional
   * Details sections).
   */
  let firstSectionEmitted = false;
  const emitSection = (id: string, body: () => void): void => {
    if (isSectionHidden(state, id)) return;
    const before = lines.length;
    if (firstSectionEmitted) lines.push("---", "");
    const beforeBody = lines.length;
    body();
    if (lines.length === beforeBody) {
      // Body emitted nothing — roll back the rule we just pushed.
      lines.length = before;
      return;
    }
    lines.push("");
  ...
  emitSection("businessOverview", () => {
    lines.push("## Business Overview", "");
    lines.push(state.oneLiner || "Not provided");
  });

  emitSection("coreServices", () => {
  ...
  emitSection("commercials", () => {
    lines.push("## Commercials & Growth", "");
    const segments = state.audienceSegments ?? [];
    if (segments.length > 0) {
      segments.forEach((segment, idx) => {
        if (idx > 0) lines.push("");
        pushAudienceSegment(lines, segment);
      });
      lines.push("");
    } else {
      lines.push(
    ...
    lines.push(formatTopGrowthChannels(state));
  });

  emitSection("usp", () => {
    lines.push("## USP & Differentiation", "");
    lines.push(`**USP:** ${state.usp || "Not provided"}`);
    if ((state.competitorsAdmire ?? "").trim() !== "") {
      lines.push(
        "",
        `**Competitors you admire or want to mimic:** ${state.competitorsAdmire}`,
      );
    }
    lines.push(
      "",
  ...
  emitSection("socialProof", () => {
    ...
    if (state.prDone === "yes" || state.prDone === "no") {
      const prDoneDisplay = state.prDone === "yes" ? "yes" : "no";
      const prSuffix =
        state.prDone === "yes" && state.prDetails
          ? ` — ${state.prDetails}`
          : "";
      lines.push("", `**PR done:** ${prDoneDisplay}${prSuffix}`);
    }
    if ((state.existingBacklinksNotes ?? "").trim() !== "") {
      lines.push(
        `**Existing backlinks to protect:** ${state.existingBacklinksNotes}`,
      );
    }
  });

  emitSection("leadMagnets", () => {
    const rows = (state.leadMagnets ?? []).filter(
      (r) =>
        (r.name ?? "").trim() !== "" ||
        (r.description ?? "").trim() !== "" ||
        (r.cta ?? "").trim() !== "",
    );
    const notes = (state.leadMagnetsNotes ?? "").trim();
  ...
  emitSection("raci", () => {
    const rows = (state.raciRows ?? []).filter(
      (r) => (r.task ?? "").trim() !== "",
    );
    const notes = (state.approvalsNotes ?? "").trim();
    if (rows.length === 0 && notes === "") return;
    lines.push("## RACI & Approvals", "");
    rows.forEach((r) => {
      const pieces: string[] = [];
      if ((r.responsible ?? "").trim()) pieces.push(`R: ${r.responsible.trim()}`);
      if ((r.accountable ?? "").trim()) pieces.push(`A: ${r.accountable.trim()}`);
      if ((r.consulted ?? "").trim()) pieces.push(`C: ${r.consulted.trim()}`);
      if ((r.informed ?? "").trim()) pieces.push(`I: ${r.informed.trim()}`);
      lines.push(`- **${r.task.trim()}:** ${pieces.join("; ")}`);
    });
    if (notes !== "") {
      if (rows.length > 0) lines.push("");
      lines.push(`**Approvals:** ${notes}`);
    }
  });

  emitSection("leadNurturing", () => {
  ...
  emitSection("additionalDetails", () => {
    if ((state.complianceNotes ?? "").trim() === "") return;
