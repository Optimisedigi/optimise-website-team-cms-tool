import { monthsActiveFrom } from './client-months-active'

export type ClientPulseScoreStatus = 'good' | 'watch' | 'risk' | 'missing' | 'not_in_scope'
export type ClientPulseTargetStatus =
  'on_track' | 'watch' | 'at_risk' | 'missing_data' | 'not_configured'
...
const ANALYTICS_METRIC_OPTIONS = new Set([
  'traffic',
  'conversions',
  'cpa',
  'revenue',
...
export async function fetchClientPulseSources(
  ...
  const [
    ...
    fetchAllPages(payload, {
      ...
      where: {
        or: [
          ...
          { updatedAt: { greater_than_equal: recentSince } },
        ],
      },
      depth: 1,
      limit,
      ...
      where: {
        and: [
          { client: { in: clientIds } },
          {
            or: [
              { updatedAt: { greater_than_equal: recentSince } },
              { status: { not_in: Array.from(TERMINAL_GOAL_STATUSES) } },
            ],
          },
        ],
      },
    ...
    fetchAllPages(payload, {
      ...
      where: {
        and: [{ client: { in: clientIds } }, { createdAt: { greater_than_equal: recentSince } }],
      },
      depth: 1,
      limit,
      sort: '-createdAt',
    ...
    fetchAllPages(payload, {
      ...
      where: {
        and: [{ client: { in: clientIds } }, { occurredAt: { greater_than_equal: recentSince } }],
      },
      depth: 1,
      limit,
      sort: '-occurredAt',
    ...
    fetchAllPages(payload, {
      ...
      where: {
        and: [{ client: { in: clientIds } }, { updatedAt: { greater_than_equal: recentSince } }],
      },
      depth: 1,
      limit,
      sort: '-updatedAt',
    ...
    fetchOptionalAllPages(
      payload,
      {
        collection: 'client-metric-snapshots',
        where: {
          and: [{ client: { in: clientIds } }, { source: { equals: 'website-we-can-quit' } }],
        },
        depth: 0,
...
export function groupClientPulseSources(
  sources: ClientPulseSources,
  clientIds: string[],
): GroupedClientPulseSources {
...
export function buildClientPulseSummary(input: {
  ...
  const budgetPacing = calculateBudgetPacing(
    input.client,
    input.grouped.googleAdsAudits.get(id) ?? [],
    adsTrend,
    now,
  )
  const clickAnomalies = calculateClickAnomalies(
    input.grouped.gscMonthlySnapshots.get(id) ?? [],
    allAdsSnapshots,
    now,
  )
  ...
  const analyticsSnapshots = stableRecords(
    input.grouped.aiVisibilitySnapshots.get(id) ?? [],
    'periodEnd',
  )
  ...
  const wcqAssessments =
    target.metric === 'assessments'
  ...
  const scheduledSignals = tasks.map((task) =>
    signalFromRecord(
      task,
      'scheduled-task',
      stringValue(task.title) || stringValue(task.name) || 'Scheduled task',
      taskStatus(task),
      stringValue(task.lastRunStatus),
      stringValue(task.nextRunAt),
    ),
  )
  const goalSignals = goalRuns.map((run) =>
    signalFromRecord(
      run,
      'goal-run',
      stringValue(run.goal) || 'Goal agent',
      goalStatus(run),
      stringValue(run.status),
      stringValue(run.nextCheckAt),
    ),
  )
  const manualSignals = ledger
    .slice(0, 6)
    .map((item) =>
      signalFromRecord(
        item,
        'ledger',
        stringValue(item.title) || 'Manual work',
        'good',
  ...
  const activitySignals = activity
    ...
    .map((item) =>
      signalFromRecord(
        item,
        'activity',
        stringValue(item.title) || stringValue(item.type) || 'Activity',
        'good',
        stringValue(item.type),
        stringValue(item.createdAt),
      ),
    )
  const reportSignals = reportSignalItems(
    siteReports,
    input.grouped.organicSnapshots.get(id) ?? [],
    input.client,
  )
  ...
  const activeGoalRuns = goalRuns.filter(
    (run) => !TERMINAL_GOAL_STATUSES.has(stringValue(run.status)),
  ).length
  const overdueItems =
    scheduledSignals.filter((item) => item.status === 'risk').length +
    goalSignals.filter((item) => item.status === 'risk').length
  const counts = {
    activeAutomations,
    activeScheduledTasks,
    activeGoalRuns,
    manualWorkLast30Days: ledger.filter((item) => (stringValue(item.occurredAt) || '') >= last30)
      .length,
    activityLast30Days: activity.filter((item) => (stringValue(item.createdAt) || '') >= last30)
      .length,
    overdueItems,
  }
  ...
  const neglect = calculateNeglectRisk(
    {
      ...
      overdueItems,
      missingReport: reportSignals.some(
        (item) => item.status === 'missing' || item.status === 'risk',
      ),
    },
    {
  ...
  const scoreHistory = shapeScoreHistory(
    input.grouped.clientPulseHistory.get(id) ?? [],
    overall,
    now,
  )
  const reasons = [
    ...targetReasons(target),
    ...organic.reasons,
    ...paidSearch.reasons,
    ...serviceCoverage.reasons,
    ...neglect.reasons,
  ]
    .filter(Boolean)
    .slice(0, 8)
  ...
  return {
    ...
    dashboardMetrics,
    ga4Sessions,
    ga4SessionsMomPercent: percentChange(
      ga4Sessions.at(-1)?.sessions ?? null,
      ga4Sessions.at(-2)?.sessions ?? null,
    ),
    organicTrend,
    adsTrend,
    budgetPacing,
...
export function calculateTargetProgress(
  ...
  if (targetValue === null || targetValue === 0) {
    return {
      ...
      comparisonWindow: targetConfig.comparisonWindow || 'last_90_days',
    }
  }
  ...
  const status: ClientPulseTargetStatus =
    rounded >= 95 ? 'on_track' : rounded >= 75 ? 'watch' : 'at_risk'
  return {
    label,
    metric,
    value: targetValue,
...
export function calculateWcqAssessmentTrend(
  ...
  const currentMonth =
    current !== null && currentMonthStartTotal !== null
      ? Math.max(0, current - currentMonthStartTotal)
      : null
  const previousMonth =
    currentMonthStartTotal !== null && previousMonthStartTotal !== null
      ? Math.max(0, currentMonthStartTotal - previousMonthStartTotal)
      : null
  const momPercent =
    currentMonth !== null && previousMonth !== null && previousMonth > 0
...
export function calculateServiceCoverage(signals: {
  ...
  const covered = Math.min(
    expected,
    signals.activeAutomations +
      signals.activeScheduledTasks +
      signals.activeGoalRuns +
      signals.manualWorkLast30Days +
      signals.reportCount,
  )
  ...
  return {
    ...
    reasons: [`${covered}/${expected} servicing signals active`],
  }
}
...
export function calculateNeglectRisk(
  ...
  if (!signals.lastMeaningfulActivityAt) {
    return {
      score: 0,
      status: 'risk',
      label: 'No meaningful activity',
      reasons: ['No recent activity, ledger, process or goal signal found'],
    }
  }
  const ageDays = Math.floor(
    (now.getTime() - new Date(signals.lastMeaningfulActivityAt).getTime()) / DAY_MS,
  )
  const penalties = (signals.overdueItems ?? 0) * 10 + (signals.missingReport ? 10 : 0)
  const freshnessScore = Math.max(
    0,
    100 - Math.max(0, ageDays - thresholds.warningDays) * 4 - penalties,
  )
  const status: ClientPulseScoreStatus =
    ageDays >= thresholds.criticalDays || freshnessScore < 45
      ? 'risk'
      : ageDays >= thresholds.warningDays || freshnessScore < 75
        ? 'watch'
        : 'good'
  return {
    score: Math.round(freshnessScore),
    status,
    label: `${ageDays}d since activity`,
    reasons: [`Last meaningful activity ${ageDays} days ago`],
  }
}
...
export function calculateOrganicTrend(monthlySnapshots: PlainRecord[], now: Date): OrganicTrend {
  ...
  if (!latestMonth)
    return { month: null, clicks: null, impressions: null, momPercent: null, yoyPercent: null }
...
export function calculateAdsTrend(adsSnapshots: PlainRecord[], now: Date): AdsTrend {
  const empty: AdsTrend = {
    month: null,
    clicks: null,
    conversions: null,
    cpa: null,
    clicksMomPercent: null,
    conversionsMomPercent: null,
    cpaMomPercent: null,
    mtdMonth: null,
    mtdClicks: null,
    mtdConversions: null,
  ...
  for (const snapshot of adsSnapshots) {
    ...
    if (label.startsWith('MTD_LY_')) {
      ...
      if (/^\d{4}-\d{2}$/.test(key) && !stringValue(snapshot.error) && !byMtdLastYear.has(key))
        byMtdLastYear.set(key, snapshot)
      continue
    }
    if (label.startsWith('MTD_')) {
      const key = label.slice(4)
      if (/^\d{4}-\d{2}$/.test(key) && !stringValue(snapshot.error) && !byMtd.has(key))
        byMtd.set(key, snapshot)
      continue
    }
    if (!label.startsWith('MONTH_')) continue
    const key = label.slice(6)
    if (!/^\d{4}-\d{2}$/.test(key) || key >= currentMonthKey || stringValue(snapshot.error))
      continue
    if (!byMonth.has(key)) byMonth.set(key, snapshot)
  }
  ...
  return {
    ...
    conversionsMomPercent: percentChange(
      latest?.conversions ?? null,
      previous?.conversions ?? null,
    ),
    ...
    mtdConversionsYoyPercent: percentChange(
      mtdTotals?.conversions ?? null,
      mtdLyTotals?.conversions ?? null,
    ),
  }
}

function deriveGa4Sessions(
  snapshots: PlainRecord[],
): Array<{ month: string; sessions: number | null }> {
  ...
  for (const snapshot of snapshots) {
    ...
    if (/^\d{4}-\d{2}$/.test(month) && !months.has(month))
      months.set(month, numberValue(snapshot.sessions))
  }
...
function deriveDashboardMetrics(
  pulse: PlainRecord,
  adsSnapshots: PlainRecord[],
  ga4Snapshots: PlainRecord[],
  metrics: Record<string, number | null | undefined>,
): ClientPulseSummary['dashboardMetrics'] {
  const configured = Array.isArray(pulse.dashboardMetrics)
    ? pulse.dashboardMetrics
        .map(recordValue)
        .filter((row) => row.enabled !== false && stringValue(row.metric))
    : []
  const rows: PlainRecord[] =
    configured.length > 0
      ? configured
      : DEFAULT_DASHBOARD_METRICS.map((metric) => ({ metric, enabled: true }))
  const currentAds = totalsForAdsSnapshot(
    adsSnapshots.find(
      (snapshot) => stringValue(snapshot.dateRangeLabel) === 'ROLLING_30D_CURRENT',
    ) ?? {},
  )
  const previousAds = totalsForAdsSnapshot(
    adsSnapshots.find(
      (snapshot) => stringValue(snapshot.dateRangeLabel) === 'ROLLING_30D_PREVIOUS',
    ) ?? {},
  )
  const currentGa4 = ga4Snapshots.find(
    (snapshot) => stringValue(snapshot.dateRangeLabel) === 'ROLLING_30D_CURRENT',
  )
  const previousGa4 = ga4Snapshots.find(
    (snapshot) => stringValue(snapshot.dateRangeLabel) === 'ROLLING_30D_PREVIOUS',
  )
  return rows.slice(0, 3).map((row) => {
    ...
    let source = 'No data'
    let invertedDelta = false
    if (metric === 'google_ads_spend') {
      value = currentAds.spend
      comparisonValue = previousAds.spend
      source = 'Google Ads, trailing 30 days'
    } else if (metric === 'google_ads_conversions') {
      value = currentAds.conversions
      comparisonValue = previousAds.conversions
      source = 'Google Ads, trailing 30 days'
    } else if (metric === 'google_ads_cost_per_lead') {
      value = currentAds.cpa
      comparisonValue = previousAds.cpa
      source = 'Google Ads spend ÷ conversions, trailing 30 days'
      invertedDelta = true
    } else if (metric === 'ga4_sessions') {
      value = numberValue(currentGa4?.sessions)
      comparisonValue = numberValue(previousGa4?.sessions)
      source = 'GA4, trailing 30 days'
    } else if (metric === 'ga4_key_events') {
      value = numberValue(currentGa4?.keyEvents)
      comparisonValue = numberValue(previousGa4?.keyEvents)
      source = 'GA4, trailing 30 days'
    } else if (metric === 'organic_clicks') {
      value = metrics.organic_clicks ?? null
      source = 'Google Search Console, latest complete month'
    } else if (metric === 'assessments') {
      value = metrics.assessments ?? null
      source = 'WeCanQuit aggregate counter'
    }
    ...
    return {
      ...
      value,
      comparisonValue,
      deltaPercent: percentChange(value, comparisonValue),
      displayValue: dashboardMetricValue(metric, value),
      source,
      invertedDelta,
    }
  })
...
function dashboardMetricLabel(metric: string): string {
  return (
    (
      {
        google_ads_cost_per_lead: 'Cost per lead',
...
function dashboardMetricValue(metric: string, value: number | null): string {
  ...
  if (metric === 'google_ads_cost_per_lead' || metric === 'google_ads_spend')
...
export function calculateBudgetPacing(
  ...
  adsTrend: AdsTrend,
  now: Date,
): ClientPulseBudgetPacing {
  const spendPolicyBudget = numberValue(recordValue(client.spendPolicy).monthlyBudgetTarget)
  const auditBudget =
    googleAdsAudits
      .map((audit) => numberValue(audit.monthlyBudget))
      .find((budget): budget is number => typeof budget === 'number' && budget > 0) ?? null
  const monthlyBudget = spendPolicyBudget && spendPolicyBudget > 0 ? spendPolicyBudget : auditBudget
  const source: ClientPulseBudgetPacing['source'] =
    spendPolicyBudget && spendPolicyBudget > 0 ? 'client' : auditBudget ? 'google_ads_audit' : null
  ...
  const expectedSpendToDate =
    Math.round(monthlyBudget * (dayOfMonthUtc(now) / daysInMonthUtc(now)) * 100) / 100
  ...
  const label =
    absDelta <= 3
      ? 'On pace'
      : `${Math.round(absDelta)}% ${deltaPercentPoints > 0 ? 'ahead' : 'below'}`
  return {
    status,
    label,
    monthlyBudget,
...
export function calculateClickAnomalies(
  gscSnapshots: PlainRecord[],
  adsSnapshots: PlainRecord[],
  now: Date,
): ClickAnomaly[] {
  return [
...
function organicClickSeries(snapshots: PlainRecord[], now: Date): ClickSeriesPoint[] {
  ...
  for (const snapshot of snapshots) {
    ...
    if (!/^\d{4}-\d{2}$/.test(key) || key >= currentMonthKey || clicks === null || byMonth.has(key))
      continue
...
function detectClickAnomaly(
  channel: ClickAnomaly['channel'],
  ...
  const status: ClientPulseScoreStatus =
    isDrop && (percentChangeValue <= -40 || (zScore ?? 0) <= -3) ? 'risk' : 'watch'
...
export function calculateOrganicScore(
  ...
  if (!servicesTracked.some((service) => ['organic', 'seo', 'content'].includes(service))) {
    return {
      score: null,
      status: 'not_in_scope',
      label: 'Organic not tracked',
      reasons: ['Organic/SEO not selected for Client Pulse'],
    }
  }
  ...
  if (trend?.month && trend.clicks !== null && trend.momPercent !== null) {
    ...
    const reasons = [
      `Organic clicks ${change >= 0 ? 'up' : 'down'} ${Math.abs(change)}% MoM (${trend.month})`,
    ]
    if (trend.yoyPercent !== null)
      reasons.push(`${trend.yoyPercent >= 0 ? '+' : ''}${trend.yoyPercent}% YoY`)
  ...
  if (!latest)
    return {
      score: null,
      status: 'missing',
      label: 'Organic missing',
      reasons: ['No organic growth snapshot found'],
    }
  const latestClicks = metricFromSnapshot(latest, 'clicks')
  const previousClicks = previous ? metricFromSnapshot(previous, 'clicks') : null
  if (latestClicks === null)
    return {
      score: null,
      status: 'missing',
      label: 'Organic missing',
      reasons: ['Latest organic snapshot has no clicks'],
    }
  if (previousClicks === null || previousClicks === 0)
    return {
      ...
      reasons: ['No previous organic snapshot for comparison'],
    }
  ...
  return {
    score,
    status,
    label: `${Math.round(change)}% organic clicks`,
    reasons: [
      `Organic clicks ${change >= 0 ? 'up' : 'down'} ${Math.abs(Math.round(change))}% vs previous snapshot`,
    ],
  }
}

function shapeScoreHistory(
  history: PlainRecord[],
  overall: ScoreSummary,
  now: Date,
): ClientPulseScoreHistoryPoint[] {
...
export async function recordClientPulseHistory(
  payload: PayloadLike,
  ...
  await Promise.all(
    summaries.flatMap(async (summary) => {
...
export function calculatePaidSearchScore(
  snapshots: PlainRecord[] | PlainRecord | undefined,
  client?: PlainRecord,
  servicesTracked: string[] = [],
): ScoreSummary {
  if (
    !servicesTracked.some((service) => ['paid_search', 'google_ads'].includes(service)) &&
    !clientHasGoogleAds(client)
  ) {
    return {
      score: null,
      status: 'not_in_scope',
      label: 'Paid search not tracked',
      reasons: ['Paid search not selected for Client Pulse'],
    }
  }
  ...
  if (list.length === 0)
    return {
      score: null,
      status: 'missing',
      label: 'Paid data missing',
      reasons: ['No Google Ads snapshot found'],
    }
  if (list.some((snapshot) => stringValue(snapshot.error)))
    return {
      score: 35,
      status: 'risk',
      label: 'Paid sync error',
      reasons: ['Latest Google Ads snapshot has an error'],
    }
  ...
  if (rows.length === 0)
    return {
      score: 50,
      status: 'watch',
      label: 'Paid data empty',
      reasons: ['Google Ads snapshots contain no rows'],
    }
  const conversions = sumRows(rows, 'conversions')
  ...
  return {
    score,
    status: conversions > 0 ? 'good' : 'watch',
    label: cpa ? `$${Math.round(cpa)} CPA` : 'Paid active',
    reasons: [`${Math.round(conversions)} conversions in latest paid snapshot`],
  }
}
...
async function fetchAllPagesWithFallback(
  payload: PayloadLike,
  args: PlainRecord,
  fallbackArgs: PlainRecord,
): Promise<PlainRecord[]> {
  try {
...
async function fetchOptionalAllPages(
  payload: PayloadLike,
  args: PlainRecord,
  label: string,
): Promise<PlainRecord[]> {
  try {
    ...
    console.error(
      `[client-pulse] optional ${label} fetch failed:`,
...
function relationshipArray(
  value: unknown,
): Array<{ id: number | string; name?: string | null; email?: string | null }> {
  ...
  return value.flatMap((item) => {
    ...
    return [
      { id, name: stringValue(record.name) || null, email: stringValue(record.email) || null },
    ]
  })
}
...
function mapClientServices(services: string[]): string[] {
  return services.map((service) =>
    service === 'google_ads' ? 'paid_search' : service === 'seo' ? 'organic' : service,
  )
}

function firstForClient(
  grouped: Map<string, PlainRecord[]>,
  id: string,
  index: number,
...
function stableRecords(records: PlainRecord[], dateField: string): PlainRecord[] {
  return [...records].sort(
    (a, b) =>
      (
        stringValue(b[dateField]) ||
        stringValue(b.updatedAt) ||
        stringValue(b.createdAt)
      ).localeCompare(
        stringValue(a[dateField]) || stringValue(a.updatedAt) || stringValue(a.createdAt),
...
function deriveMetrics(
  latestOrganic: PlainRecord | undefined,
  adsSnapshots: PlainRecord[],
  analyticsSnapshot?: PlainRecord,
  organicTrend?: OrganicTrend,
  ...
  const organicClicks =
    organicTrend?.clicks ?? (latestOrganic ? metricFromSnapshot(latestOrganic, 'clicks') : null)
  const analyticsTraffic = analyticsSnapshot
    ? metricFromSnapshot(analyticsSnapshot, 'totalSessions')
    : null
  const analyticsConversions = analyticsSnapshot
    ? metricFromSnapshot(analyticsSnapshot, 'totalConversions')
    : null
  const analyticsRevenue = analyticsSnapshot
    ? metricFromSnapshot(analyticsSnapshot, 'conversionValue')
    : null
...
function automationSignalItems(client: PlainRecord, servicesTracked: string[]): SignalItem[] {
  ...
  const add = (id: string, label: string, enabled: boolean, inScope = true): void => {
    signals.push({
      id,
      label,
      status: inScope ? (enabled ? 'good' : 'missing') : 'not_in_scope',
  ...
  add(
    ...
    gads.performanceReportEnabled === true ||
      recordValue(gads.weeklyReport).weeklyReportEnabled === true,
    paidScope,
  )
  const organicScope = servicesTracked.some((service) =>
    ['organic', 'seo', 'content'].includes(service),
  )
  add('seo-health', 'Monthly SEO health', seo.monthlyHealthEnabled === true, organicScope)
  add(
    'serp-monitor',
    'SERP monitor',
    recordValue(client.serpMonitor).enabled === true,
    organicScope,
  )
...
function signalFromRecord(
  ...
  return {
    id: `${prefix}-${normalizeId(record.id) || label}`,
    label,
    status,
    detail,
    at: at ?? null,
...
function reportSignalItems(
  siteReports: PlainRecord[],
  organicSnapshots: PlainRecord[],
  client: PlainRecord,
): SignalItem[] {
  const gads = recordValue(client.gadsAuto)
  ...
  items.push(
    siteReports[0]
      ? signalFromRecord(
          ...
          'Site health report',
          'good',
          stringValue(siteReports[0].auditStatus),
          stringValue(siteReports[0].reportDate),
        )
      : {
          id: 'site-health-missing',
          label: 'Site health report',
          status: 'missing',
  ...
  items.push(
    organicSnapshots[0]
      ? signalFromRecord(
          organicSnapshots[0],
          'organic-report',
          'Organic growth snapshot',
          'good',
          undefined,
          stringValue(organicSnapshots[0].snapshotDate),
        )
      : {
          id: 'organic-snapshot-missing',
          label: 'Organic growth snapshot',
          status: 'missing',
          detail: 'No snapshot found',
        },
  )
  items.push({
    id: 'paid-report-config',
    label: 'Paid report automation',
    status:
      gads.performanceReportEnabled === true ||
      recordValue(gads.weeklyReport).weeklyReportEnabled === true
        ? 'good'
        : 'missing',
    detail: 'Google Ads report config',
  })
  return items
}

function qbrSignalItems(ledger: PlainRecord[], activity: PlainRecord[]): SignalItem[] {
  const qbr = [...ledger, ...activity].find((item) =>
    /\b(qbr|quarterly business review)\b/i.test(
      `${stringValue(item.title)} ${stringValue(item.summary)} ${stringValue(item.type)}`,
    ),
  )
  return qbr
    ? [
        signalFromRecord(
          ...
          'QBR signal',
          'good',
          stringValue(qbr.title),
          stringValue(qbr.occurredAt) || stringValue(qbr.createdAt),
        ),
      ]
    : [
        {
          ...
          label: 'QBR signal',
          status: 'missing',
          detail: 'No QBR signal in recent activity',
        },
      ]
}

function negativeKeywordSignalItems(activity: PlainRecord[], client: PlainRecord): SignalItem[] {
  const recent = activity.find((item) =>
    NEGATIVE_KEYWORD_ACTIVITY_TYPES.has(stringValue(item.type)),
  )
  ...
  if (recent)
    return [
      signalFromRecord(
        ...
        'good',
        stringValue(recent.type),
        stringValue(recent.createdAt),
      ),
    ]
  return [
    {
      id: 'negative-keyword-config',
      label: 'Negative keyword activity',
      status: enabled ? 'watch' : 'missing',
      detail: enabled ? 'Automation enabled; no recent activity' : 'Negative sweep not enabled',
    },
  ]
}

function calculateOverallScore(scores: ScoreSummary[]): ScoreSummary {
  const numeric = scores.filter((score) => typeof score.score === 'number') as Array<
    ScoreSummary & { score: number }
  >
  const score =
    numeric.length > 0
      ? Math.round(numeric.reduce((sum, item) => sum + item.score, 0) / numeric.length)
      : null
  const status: ClientPulseScoreStatus = scores.some((item) => item.status === 'risk')
    ? 'risk'
    : scores.some((item) => item.status === 'watch' || item.status === 'missing')
      ? 'watch'
      : 'good'
  return {
    score,
    status,
    label: 'Overall pulse',
    reasons: scores.flatMap((item) => item.reasons).slice(0, 4),
  }
}
...
function selectedAnalyticsMetrics(pulse: PlainRecord): string[] {
  const selected = stringArray(pulse.analyticsMetrics).filter((metric) =>
    ANALYTICS_METRIC_OPTIONS.has(metric),
  )
...
function clientHasGoogleAds(client?: PlainRecord): boolean {
  return (
    !!client &&
    (stringValue(client.googleAdsCustomerId) !== '' ||
      stringArray(client.services).includes('google_ads'))
  )
}

function sortSummary(a: ClientPulseSummary, b: ClientPulseSummary): number {
  const riskRank: Record<ClientPulseScoreStatus, number> = {
    risk: 0,
    watch: 1,
    missing: 2,
    good: 3,
  ...
  return (
    riskRank[a.scores.overall.status] - riskRank[b.scores.overall.status] ||
