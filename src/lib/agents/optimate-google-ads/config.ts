...
const TOOL_INVENTORY = [
  ...
  "- Budget management/email asks use get_budget_management_email. Monthly email dashboard component asks use get_dashboard_email_components before the canonical budget tracker. One-off Gmail draft asks use create_gmail_draft. Recurring report asks use propose_scheduled_task.",
...
Treat weekly budget management emails as a separate client report from monthly budget management emails. Weekly budget reports MUST include a 1 to 2 sentence plain Gmail-safe intro above the report with one positive data point from the weekly rows, and the Gmail subject MUST be "[Client Name] - Google Ads Weekly Report". Default weekly reports to the last 4 completed Monday-Sunday weeks, unless the user requests a different number of weeks. Monthly/MTD budget emails include selected dashboard component blocks above the budget tracker when the user selected OptiMate chips. If no selected components are present, default monthly budget emails to a Monthly Performance Trend for the last 6 completed calendar months. Keep the subject returned by get_budget_management_email.
...
If the budget email request asks for a 1 to 3 sentence summary on top around weekly/recent performance, call get_weekly_metric_table with metrics=["spend","conversions","cpa"] first. Pick the strongest truthful story from CPA, conversions, CTR, or avg CPC only if supported by the weekly rows. Then call get_budget_management_email with mode='this_month'. For Gmail drafts, call create_gmail_draft with htmlBody = a plain, non-bold "Hey team," greeting, the plain Gmail-safe summary paragraph, then the budget HTML verbatim. The greeting and summary paragraph must be full-width Gmail text: use paragraph styles like "margin:0 0 20px;width:100%;max-width:none;display:block" and do not place them inside the table wrapper.

If the request is specifically for a weekly budget report, call get_weekly_metric_table with metrics=["spend","conversions","cpa"], weeks=4 by default (or the user-requested number of weeks), and endDate set to the previous Sunday in agency time so every row is a completed Monday-Sunday week. Use the returned rows as evidence for the positive intro sentence. Then call get_budget_management_email with mode='this_month', and create the draft with subject "[Client Name] - Google Ads Weekly Report" and htmlBody = plain, non-bold "Hey team," greeting, intro paragraph, weekly table HTML, then budget HTML. The greeting and intro paragraph must be full-width Gmail text: use paragraph styles like "margin:0 0 20px;width:100%;max-width:none;display:block" and do not place them inside the table wrapper.

If the request is specifically for a monthly budget report and includes selected dashboard component keys (monthly_performance, kpi_summary, top_converters, budget_wasters, campaign_breakdown, lead_quality, competitor_snapshot), call get_dashboard_email_components with those components in the selected order and the requested months/range. Then call get_budget_management_email with mode='this_month'. For Gmail drafts, call create_gmail_draft with htmlBody = plain, non-bold "Hey team," greeting, optional summary paragraph if requested, selected component HTML from get_dashboard_email_components.data.html, then the canonical budget HTML.

If the request is specifically for a monthly budget report and does NOT include selected dashboard component keys, call get_dashboard_email_components with components=["monthly_performance"] and months=6 by default, or the user-requested number of months. Then call get_budget_management_email with mode='this_month'. For Gmail drafts, call create_gmail_draft with htmlBody = plain, non-bold "Hey team," greeting, optional summary paragraph if requested, monthly component HTML, then budget HTML. Respect user-requested number of months/weeks.
