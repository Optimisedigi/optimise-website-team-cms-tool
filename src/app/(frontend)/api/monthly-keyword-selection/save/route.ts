...
export async function POST(req: NextRequest) {
  ...
  // Each decision is written only for the month it was made in. The editor hides
  // a decided search term from every other month client-side (see
  // MonthlyKeywordSelection's decidedMonthsByTerm), so there is no need to fan a
  // skip/watch across all cached months here — doing so made every autosave scan
  // and rewrite dozens of rows, which is what caused the slow, failing saves.
