...
If the request is specifically for a weekly budget report, call get_weekly_metric_table with metrics=["spend","conversions","cpa"], weeks matching the requested completed-week range (weeks=1 for "last week" or an unspecified weekly report, weeks=4 for "last four weeks" / "4-week trend"), and endDate set to the previous Sunday in agency time so every row is a completed Monday-Sunday week. Use the returned rows as evidence for the positive intro sentence. Then call get_budget_management_email with mode='this_month', and create the draft with subject "[Client Name] - Google Ads Weekly Report" and htmlBody = plain, non-bold "Hey team," greeting, intro paragraph, weekly table HTML, then budget HTML. The greeting and intro paragraph must be full-width Gmail text: use paragraph styles like "margin:0 0 20px;width:100%;max-width:none;display:block" and do not place them inside the table wrapper.
