...
type AdCopySet = { headlines: string[]; descriptions: string[] }
...
function AdPreview({ headlines, descriptions, url }: { headlines: string[]; descriptions: string[]; url: string }) {
    ...
    try {
      const res = await fetch(`/api/google-ads-audits/${id}`, {
        ...
        body: JSON.stringify({
          ...
          ...(publishToggle && !adCopyPublished ? { adCopyStatus: 'published' } : {}),
        }),
      })
      if (res.ok) {
                ...
                >
                  <div>
                    ...
                    <span style={{ marginLeft: 8, fontSize: 11, color: '#9ca3af' }}>
                      {copy.headlines?.length || 0}h {copy.descriptions?.length || 0}d
                    </span>
                    {agComments.length > 0 && (
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      {(copy.headlines || []).map((h, i) => (
                        <div key={i} style={{ display: 'flex', gap: 4, marginBottom: 3, alignItems: 'center' }}>
                          ...
                          />
                          <span style={{ fontSize: 10, color: h.length > 30 ? '#ef4444' : '#9ca3af', minWidth: 35 }}>
                            {h.length}/30
                          </span>
                        </div>
                      ))}
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      {(copy.descriptions || []).map((d, i) => (
                        <div key={i} style={{ display: 'flex', gap: 4, marginBottom: 3, alignItems: 'center' }}>
                          ...
                          />
                          <span style={{ fontSize: 10, color: d.length > 90 ? '#ef4444' : '#9ca3af', minWidth: 35 }}>
                            {d.length}/90
                          </span>
                        </div>
                      ))}
