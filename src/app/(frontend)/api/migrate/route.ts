...
export async function GET(request: NextRequest) {
  ...
  // ── Google Ads snapshots + Goal agent audit trail (2026-06 emergency schema heal) ──
  ...
  await run("goal_runs.next_check_at", "ALTER TABLE `goal_runs` ADD `next_check_at` text");
  await run("goal_runs.cooling_off_until", "ALTER TABLE `goal_runs` ADD `cooling_off_until` text");
  await run("goal_runs.iterations_count", "ALTER TABLE `goal_runs` ADD `iterations_count` integer NOT NULL DEFAULT 0");
  await run("goal_runs.parameters", "ALTER TABLE `goal_runs` ADD `parameters` text");
  ...
  await run("locked_docs_rels.goal_risk_tiers_id", "ALTER TABLE `payload_locked_documents_rels` ADD `goal_risk_tiers_id` integer REFERENCES `goal_risk_tiers`(`id`) ON DELETE cascade");
