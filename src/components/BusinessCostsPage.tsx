    ...
    <div style={{ padding: '20px 0' }}>
      ...
      <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {sorted.length === 0 ? (
              <tr><td colSpan={5} style={{ padding: '20px 0', textAlign: 'center', color: 'var(--theme-elevation-400)' }}>No transactions</td></tr>
            ) : sorted.map((tx) => {
              const cat = typeof tx.category === 'object' && tx.category ? tx.category as Category : null
              return (
                <tr key={String(tx.id)} style={{ borderBottom: '1px solid var(--theme-elevation-100)' }}>
        ...
        {history.map((entry) => {
          ...
          return (
            <div key={entry.month} style={{ width: `${barWidth}%`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
              <div style={{ fontSize: 10, fontWeight: 600, marginBottom: 2, color: 'var(--theme-elevation-500)' }}>${entry.total.toFixed(0)}</div>
              <div style={{ width: '60%', display: 'flex', flexDirection: 'column' }}>
                {segments.map((seg) => {
                  const h = (seg.total / maxTotal) * (chartHeight - 30)
                  return (
                    <div
                      key={String(seg.id)}
                      title={`${seg.name}: $${seg.total.toFixed(2)}`}
                      style={{ height: Math.max(h, 2), background: seg.color, borderRadius: 0 }}
                    />
                  )
                })}
              </div>
