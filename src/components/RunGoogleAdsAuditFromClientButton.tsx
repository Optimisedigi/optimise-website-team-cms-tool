...
const RunGoogleAdsAuditFromClientButton = () => {
  ...
  useEffect(() => {
    if (!id || !customerId) { setCurrent(null); return }
    let cancelled = false
    fetch(`/api/google-ads-audits?${auditQuery()}`, { credentials: 'include' })
      .then((response) => response.ok ? response.json() : null)
      .then((found) => { if (!cancelled) setCurrent(found?.docs?.[0] ?? null) })
      .catch(() => undefined)
  ...
  const handleClick = async () => {
    ...
    setResult(null)
    try {
      const foundResponse = await fetch(`/api/google-ads-audits?${auditQuery()}`, { credentials: 'include' })
      ...
      setCurrent({ ...audit, snapshotState: snapshot.status, snapshotPeriodStart: snapshot.periodStart, snapshotPeriodEnd: snapshot.periodEnd })
  ...
  return (
    <section style={{ marginBottom: 20 }} aria-labelledby="client-audit-snapshot-title">
      ...
      {current && <p style={{ marginTop: 8, fontSize: 13, color: 'var(--theme-elevation-600)' }}>
        Current snapshot: {current.snapshotState || 'not created'}{current.snapshotPeriodStart && current.snapshotPeriodEnd ? `. Frozen window: ${String(current.snapshotPeriodStart).slice(0, 10)} to ${String(current.snapshotPeriodEnd).slice(0, 10)}.` : '.'}
      </p>}
