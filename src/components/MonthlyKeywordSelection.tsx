    ...
    <div className="od-fullbleed-tool" style={{ color: 'var(--theme-text)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(420px, 1fr) minmax(360px, 0.9fr)', gap: 12, alignItems: 'start', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', minHeight: 66, padding: 12, border: '1px solid var(--theme-elevation-150)', borderRadius: 8 }}>
          <button type="button" className="btn btn--style-primary" onClick={applyApproved} disabled={approvedCount === 0 || applying || unsavedKeys.size > 0} title={unsavedKeys.size > 0 ? 'Waiting for auto-save to finish before applying' : undefined} style={{ margin: 0, whiteSpace: 'nowrap', flexShrink: 0 }}>{applying ? 'Saving…' : appliedJustNow ? 'Saved ✓' : `Apply ${approvedCount} added negative${approvedCount === 1 ? '' : 's'}`}</button>
