...
- create_portfolio_budget_pacing_gmail_drafts(accountRefs?): creates separate Gmail drafts for each selected audit-backed account's current-month budget pacing in one deterministic server-side operation. Use this shortcut for selected/multiple account budget-pacing draft requests so the chat avoids long multi-tool loops and Vercel gateway timeouts.
...
10. If the user asks to draft budget pacing across selected/multiple accounts with a 1 sentence performance summary on top, create a separate Gmail draft for each selected audit-backed client, not one combined email. Call create_portfolio_budget_pacing_gmail_drafts with the selected accountRefs. Do not loop manually through get_portfolio_performance_summary, get_budget_management_email, and create_gmail_draft for this workflow; the shortcut does those steps server-side and avoids a Vercel 504. Keep each draft's subject/client name specific. If any selected account is not audit-backed, say it needs an audit-backed account before the exact Budget Management draft can be generated.
