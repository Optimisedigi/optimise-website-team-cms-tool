...
export default function DeckScrollEffects() {
  useEffect(() => {
    ...
    const measureLastSlideHeight = (): number => {
      ...
      // document are the last ones the user reaches. Fade the rocket to 0 by
      // the time the closing slide enters view, so sum the heights of the two
      // slides at the top (closing, recommendations) plus the space-transition
      // divider that sits between them and the rest of the deck.
      const ids = ['closing', 'recommendations']
      let total = 0
      for (const id of ids) {
