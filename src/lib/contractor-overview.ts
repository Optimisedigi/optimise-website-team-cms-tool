...
export type ContractorFortnightPayment = {
  id: string;
  contractorId: number;
...
export type ContractorOverview = {
  contractors: unknown[];
  fortnightlyPayments: ContractorFortnightPayment[];
  globals: {
...
async function findAllDocs(payload: { find: (options: Record<string, unknown>) => Promise<{ docs: unknown[]; totalPages?: number }> }, options: Record<string, unknown>): Promise<any[]> {
  const docs: any[] = [];
  let page = 1;
  let totalPages = 1;
...
/** Compact, auto-derived transfer-management data for Contractor Costs and InvoiceMate. */
export async function loadContractorOverview(payload: {
  find: (options: any) => Promise<{ docs: unknown[]; totalPages?: number }>;
  findByID: (options: any) => Promise<unknown>;
  ...
  const fortnightlyPayments: ContractorFortnightPayment[] = [];
  ...
  return { contractors: contractorsRows, fortnightlyPayments, globals };
}

export function filterContractorPayments(
  payments: ContractorFortnightPayment[],
  opts: { contractorName?: string; status?: "paid" | "unpaid" } = {},
): ContractorFortnightPayment[] {
  const needle = opts.contractorName?.trim().toLowerCase();
  return payments.filter((payment) => {
    if (opts.status && payment.status !== opts.status) return false;
    if (!needle) return true;
    return payment.contractorName.toLowerCase().includes(needle);
  });
}
