...
const RunGoogleAdsAuditFromProposalButton = () => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      // If proposal is linked to a client, link the audit too
      const clientField = fields?.client?.value
      if (clientField) {
        // Could be an object (populated) or a number/string (ID)
        body.client = typeof clientField === 'object' && clientField !== null
          ? (clientField as any).id || (clientField as any).value
          : clientField
      }
...
export default RunGoogleAdsAuditFromProposalButton
