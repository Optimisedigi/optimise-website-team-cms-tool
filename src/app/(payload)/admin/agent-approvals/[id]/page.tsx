/**
 * Approval review page. Rendered inside the Payload admin shell (sidebar +
 * header). Two-column layout: left shows the rendered preview (clientHtml in
 * iframe srcdoc, internalMarkdown rendered with a small helper), right shows
 * the structured payload, run link, agent + client metadata. Action buttons
 * live in the client `Actions` component.
 */
...
export default async function ApprovalReviewPage({
  ...
  if (!user) {
    redirect(`/admin/login?redirect=${encodeURIComponent(`/admin/agent-approvals/${id}`)}`);
  }
      ...
      <div className="gutter--left gutter--right">
        <div style={PAGE_STYLE}>
          <div style={{ marginBottom: 12 }}>
            <a href="/admin/agent-approvals" style={{ fontSize: 12, color: CMS_BLUE, textDecoration: "none", fontWeight: 700 }}>
              ← Approvals
            </a>
          </div>
          <div
              ...
              <div style={CARD}>
                ...
                <dl style={{ fontSize: 13, margin: 0 }}>
                  ...
                  <Meta label="Run" value={
                    <a href={`/admin/agent-runs/${doc.agentRunId}`} style={{ color: CMS_BLUE, fontWeight: 700 }}>
                      {doc.agentRunId} →
                    </a>
                  } />
