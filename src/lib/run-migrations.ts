...
export async function runMigrations(
  ...
  try {
    ...
    await run("clients_client_pulse_dashboard_metrics", `CREATE TABLE IF NOT EXISTS \`clients_client_pulse_dashboard_metrics\` (\`_order\` integer NOT NULL, \`_parent_id\` integer NOT NULL, \`id\` text PRIMARY KEY NOT NULL, \`metric\` text NOT NULL, \`label\` text, \`enabled\` integer DEFAULT true, FOREIGN KEY (\`_parent_id\`) REFERENCES \`clients\`(\`id\`) ON UPDATE no action ON DELETE cascade)`);
    await run("clients_client_pulse_dashboard_metrics_order_idx", "CREATE INDEX IF NOT EXISTS `clients_client_pulse_dashboard_metrics_order_idx` ON `clients_client_pulse_dashboard_metrics` (`_order`)");
    await run("clients_client_pulse_dashboard_metrics_parent_id_idx", "CREATE INDEX IF NOT EXISTS `clients_client_pulse_dashboard_metrics_parent_id_idx` ON `clients_client_pulse_dashboard_metrics` (`_parent_id`)");
    await run("client_analytics_snapshots", `CREATE TABLE IF NOT EXISTS \`client_analytics_snapshots\` (\`id\` integer PRIMARY KEY NOT NULL, \`client_id\` integer NOT NULL, \`source\` text DEFAULT 'ga4' NOT NULL, \`date_range_label\` text NOT NULL, \`period_start\` text NOT NULL, \`period_end\` text NOT NULL, \`sessions\` numeric, \`key_events\` numeric, \`conversions\` numeric, \`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL, \`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL, FOREIGN KEY (\`client_id\`) REFERENCES \`clients\`(\`id\`) ON UPDATE no action ON DELETE cascade)`);
    await run("client_analytics_snapshots_client_idx", "CREATE INDEX IF NOT EXISTS `client_analytics_snapshots_client_idx` ON `client_analytics_snapshots` (`client_id`)");
    await run("client_analytics_snapshots_label_idx", "CREATE INDEX IF NOT EXISTS `client_analytics_snapshots_label_idx` ON `client_analytics_snapshots` (`date_range_label`)");
    ...
    // ── google_ads_automation_events (2026-09-03) ──────────────────────────
    // Persisted Google Ads change_event rows. The API only serves the trailing
    // ~30 days, so the daily cron writes here or the history is lost.
    // resource_name is the dedupe key and is uniquely indexed.
    await run("google_ads_automation_events", "CREATE TABLE IF NOT EXISTS `google_ads_automation_events` (`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL, `client_id` integer, `customer_id` text, `change_date_time` text, `resource_name` text NOT NULL, `change_resource_type` text, `resource_change_operation` text, `client_type` text, `user_email` text, `campaign_id` text, `campaign_name` text, `changed_fields` text, `old_values` text, `new_values` text, `is_google_automated` integer DEFAULT false, `summary` text, `impact_spend_before` numeric, `impact_spend_after` numeric, `impact_conv_before` numeric, `impact_conv_after` numeric, `impact_computed_at` text, `review_status` text DEFAULT 'unreviewed', `related_approval_id` integer, `updated_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL, `created_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL)");
    await run("google_ads_automation_events_resource_name_idx", "CREATE UNIQUE INDEX IF NOT EXISTS `google_ads_automation_events_resource_name_idx` ON `google_ads_automation_events` (`resource_name`)");
    await run("google_ads_automation_events_client_idx", "CREATE INDEX IF NOT EXISTS `google_ads_automation_events_client_idx` ON `google_ads_automation_events` (`client_id`)");
    await run("google_ads_automation_events_customer_id_idx", "CREATE INDEX IF NOT EXISTS `google_ads_automation_events_customer_id_idx` ON `google_ads_automation_events` (`customer_id`)");
    ...
    // ── google_search_status_incidents (2026-09-03) ────────
    // Official Google Search Status Dashboard incidents. incident_id is the
    // unique dedupe key so daily polls upsert rather than duplicate.
    await run("google_search_status_incidents", "CREATE TABLE IF NOT EXISTS `google_search_status_incidents` (`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL, `incident_id` text NOT NULL, `title` text NOT NULL, `kind` text DEFAULT 'other' NOT NULL, `begin` text, `end` text, `modified` text, `status_impact` text, `severity` text, `service_name` text, `latest_update` text, `source_uri` text, `notified_at` text, `raw` text, `updated_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL, `created_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL)");
    await run("google_search_status_incidents_incident_id_idx", "CREATE UNIQUE INDEX IF NOT EXISTS `google_search_status_incidents_incident_id_idx` ON `google_search_status_incidents` (`incident_id`)");
    await run("google_search_status_incidents_kind_idx", "CREATE INDEX IF NOT EXISTS `google_search_status_incidents_kind_idx` ON `google_search_status_incidents` (`kind`)");
    await run("google_search_status_incidents_begin_idx", "CREATE INDEX IF NOT EXISTS `google_search_status_incidents_begin_idx` ON `google_search_status_incidents` (`begin`)");
    ...
    ORDER BY s.\`_order\` ASC`, ["no such table"]);
    ...
    // ── Hosting subscription billing (2026-08-14) ──────────────────────
    // Keep in sync with src/migrations/20260814_120000_add_hosting_billing.ts.
    for (const column of [
      ...
      await run(
        `clients.${column[0]}`,
        `ALTER TABLE \`clients\` ADD COLUMN \`${column[0]}\` ${column[1]}`,
      );
    }

    await run("hosting_billing_settings", `CREATE TABLE IF NOT EXISTS \`hosting_billing_settings\` (
      \`id\` integer PRIMARY KEY NOT NULL, \`currency\` text DEFAULT 'aud' NOT NULL,
      \`card_surcharge_percentage\` numeric DEFAULT 0 NOT NULL, \`card_surcharge_fixed_cents\` numeric DEFAULT 0 NOT NULL,
      \`surcharge_effective_from\` text, \`minimum_notice_days\` numeric DEFAULT 30 NOT NULL,
      \`capacity_change_clause\` text DEFAULT 'If hosting capacity exceeds the included allowance, we may propose a future price change with written notice. The change will take effect at a future renewal only.' NOT NULL,
      \`notice_email_subject\` text DEFAULT 'Hosting capacity price change notice for {{clientName}}' NOT NULL,
      \`notice_email_body\` text DEFAULT 'Hello {{clientName}},\n\nWe are proposing a hosting price change from {{currentPrice}} to {{newPrice}}, effective on {{effectiveDate}}. Reason: {{reason}}.' NOT NULL,
      \`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
      \`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
    )`);
    await run("hosting_billing_settings_plans", `CREATE TABLE IF NOT EXISTS \`hosting_billing_settings_plans\` (
      ...
      \`monthly_base_cents\` numeric NOT NULL, \`annual_base_cents\` numeric NOT NULL, \`active\` boolean DEFAULT true,
    ...
    await run("clients_hosting_subscription_price_changes", `CREATE TABLE IF NOT EXISTS \`clients_hosting_subscription_price_changes\` (
      \`_order\` integer NOT NULL, \`_parent_id\` integer NOT NULL, \`id\` text PRIMARY KEY NOT NULL,
      \`status\` text, \`reason\` text, \`effective_at\` text, \`old_quote\` text, \`new_quote\` text,
      \`notice_sent_at\` text, \`notice_message_id\` text, \`applied_at\` text, \`stripe_reference\` text,
      \`last_error\` text, \`retry_count\` numeric,
      FOREIGN KEY (\`_parent_id\`) REFERENCES \`clients\`(\`id\`) ON UPDATE no action ON DELETE cascade
    )`);
    await run("clients_hosting_subscription_price_changes.applied_at", "ALTER TABLE `clients_hosting_subscription_price_changes` ADD COLUMN `applied_at` text");
