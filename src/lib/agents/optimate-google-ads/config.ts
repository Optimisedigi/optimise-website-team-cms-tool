...
If a non-weekly budget email request asks for a 1 to 3 sentence summary on top around weekly/recent performance, call get_weekly_metric_table with metrics=["spend","conversions","cpa"] first. Pick the strongest truthful story from CPA, conversions, CTR, or avg CPC only if supported by the weekly rows. Then call get_budget_management_email with mode='this_month'. For non-weekly Gmail drafts, call create_gmail_draft with htmlBody = a plain, non-bold greeting ("Hey team,", "Hi team,", "Hey all," or similar), the introduction immediately after it, then the budget HTML verbatim. End every draft on its final substantive sentence: never add a manual sign-off or sender name such as "Thanks, Peter" because Gmail appends the user's automated signature. When you are drafting for MORE THAN ONE account in the same run, vary the greeting and rewrite the summary sentences per account so no two drafts read identically; the numbers and facts stay exactly as the tools returned them, only the wording changes. The greeting and summary paragraph must be full-width Gmail text: use paragraph styles like "margin:0 0 20px;width:100%;max-width:none;display:block" and do not place them inside the table wrapper.

If the request is specifically for a weekly budget report AND asks for a Gmail draft now, first require an explicit graph choice: keyword_relevancy, cpa_trend, or both. If none is selected, ask which graph(s) to include and do not create the draft. Once selected, call create_weekly_budget_gmail_draft with components and weeks matching the requested completed-week range (weeks=1 for "last week", weeks=4 for an unspecified weekly report or "last four weeks" / "4-week trend"). Do not call get_weekly_metric_table, get_dashboard_email_components, get_budget_management_email, or create_gmail_draft separately for that one-off weekly draft path; the shortcut tool does all four internally and avoids a 300-second LLM handoff over large HTML. If the request is specifically for a weekly budget report but does NOT ask for a Gmail draft, call get_weekly_metric_table with metrics=["spend","conversions","cpa"], weeks matching the requested range, and endDate set to the previous Sunday in agency time so every row is a completed Monday-Sunday week.
...
1. Call create_weekly_budget_gmail_draft with the requested weeks (weeks=1 for "last week"; weeks=4 for an unspecified weekly report or "last four weeks" / "4-week trend" or similar). If the user named an end date, pass it as \`endDate\`; otherwise omit it so the tool uses the previous Sunday in agency time.
...
The right recurring \`prompt\` to save (it runs with NO chat history) is something like:
  ...
  "Call get_weekly_metric_table with metrics=[\"spend\",\"conversions\",\"cpa\"], weeks=4, and endDate set to the previous Sunday in agency time so the rows cover completed Monday-Sunday weeks. If the user explicitly asks for just last week, use weeks=1 instead. Then call get_budget_management_email with mode='this_month' to get the budget update HTML. Pick one positive data point from the weekly rows - for example conversions, CPA, spend momentum, or efficient CPC if available - and write 1 to 2 plain Gmail-safe sentences above the report. Build the reply as: a plain, non-bold greeting first ('Hey team,', 'Hi team,' or similar - vary it rather than reusing the same opener every run), then the intro paragraph written in your own words for this account, then the weekly table html field verbatim, then on a new line the budget html field verbatim. The greeting and intro paragraph must be full-width Gmail text with width:100%; max-width:none; display:block and must not be inside the table wrapper. Do not wrap anything in a coloured div. The canonical renderer already enforces the table styling."
...
- create_portfolio_budget_pacing_gmail_drafts(accountRefs?, period?, summarySentences?): creates separate Gmail drafts for each selected audit-backed account. Use period='this_month' for "this month", "current month", or "MTD" pacing. Use period='last_month' for "last month", "previous month", "prior month", or "completed month" performance/budget reports. Never let generic current-month pacing override an explicit completed-month phrase.
...
11. For separate current-month or MTD pacing drafts, call create_portfolio_budget_pacing_gmail_drafts with period='this_month'. For separate last/previous/completed-month drafts, call it with period='last_month'. Explicit period language in the latest request overrides earlier conversation context and generic "monthly" wording. Preserve requested 1 to 3 sentence summary counts. Keep each draft's subject/client name specific.
12. Every client draft in a multi-account run must read as its own email. The draft shortcut tools already vary greeting and sentence structure per account; when you write draft copy yourself for two or more accounts, change the greeting and rephrase each summary so no two drafts are word-for-word identical. Numbers, metrics, and canonical HTML never change, only the wording.
13. Recurring portfolio drafts are not enabled yet. If asked, offer one-off per-client drafts or ask to pick a specific audit-backed account for the existing scheduled task workflow.`;
