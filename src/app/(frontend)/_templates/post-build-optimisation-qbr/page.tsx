/**
 * ============================================================================
 *  TEMPLATE: Post-Build Optimisation / QBR Presentation
 * ============================================================================
 *
 *  This folder is the canonical template for client-facing decks that follow
 *  the post-build-optimisation / Quarterly Business Review pattern. It lives
 *  in (frontend)/_templates/ — the leading underscore tells Next.js NOT to
 *  expose this as a live route, so the template stays private.
 *
 *  Reference instance (already populated with Berendsen + MTP May 2026 data):
 *    (frontend)/partners/google-ads-audit/team-session-may-2026/page.tsx
 *
 *  HOW TO START A NEW DECK
 *  -----------------------
 *    1. Copy this folder to (frontend)/partners/<area>/<slug>/
 *       e.g. (frontend)/partners/google-ads-audit/acme-aug-2026/
 *    2. Rename ids in the SLIDES array if you change slide composition.
 *    3. Update the data in each <Slide ...> block. Numbers come from the
 *       same sources used here (GA4, Google Ads landing_page_view, CMS
 *       audits, search-term reports). Do NOT invent numbers.
 *    4. Update the timeline + agenda copy on slides 1 and 2.
 *    5. Run locally (npm run dev, port 3004) and review every slide before
 *       sharing. The deck publishes only when committed + pushed to main.
 *
 *  STRUCTURE
 *  ---------
 *    Section 1: Title + agenda
 *    Section 2: Paid Search 101 (educational, only include if non-paid
 *               stakeholders are in the room)
 *    Section 3: What we shipped in month one
 *    Section 4: Historical traffic + paid CPC charts (per business)
 *    Section 5: Conversion mix + business-validation slide
 *    Section 6: Cash-sales / revenue cross-check (if client provides data)
 *    Section 7: Optimisations identified (LP, structural, brand, budget)
 *    Section 8: How we track progress + tracking caveats
 *    Section 9: 90-day target + Q&A
 *
 *  CONVENTIONS
 *  -----------
 *    - No en-dashes / em-dashes in slide copy (client-facing rule).
 *    - Slide primitives are defined inline at the top of this file:
 *      Slide, SlideHeading, SlideSubtext, Card, DataTable, StatCard, Pill,
 *      LineChart, DualAxisLineChart, HorizontalBarChart.
 *    - Light/dark slide rhythm: alternate dark/light props for visual variety.
 *    - Print stylesheet (zoom 0.85) keeps long slides on one page.
