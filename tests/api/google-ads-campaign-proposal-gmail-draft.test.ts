...
describe('Google Ads campaign proposal Gmail draft route', () => {
        ...
        proposedCampaigns: [
          {
            ...
            adGroups: [
              {
                name: 'Acme Plumbing Brand',
                totalMonthlyVolume: 260,
                keywords: [{ text: 'acme plumbing' }, { text: 'acme plumber' }],
                landingPage: { url: '/', status: 'create' },
              },
            ],
          },
        ],
  ...
  it('falls back to Gmail-safe inline proposal HTML when no body is supplied', async () => {
    ...
    expect(mockCreateGmailDraft).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        subject: '[Acme Plumbing] campaign structure proposal',
        htmlBody: expect.stringContaining('border-collapse:collapse'),
      }),
    )
    const htmlBody = mockCreateGmailDraft.mock.calls[0][1].htmlBody as string
    expect(htmlBody).toContain('Recommended Campaign Structure')
    expect(htmlBody).toContain('background:#eef6ff')
    expect(htmlBody).toContain('Acme Plumbing Brand')
  })
})
